...
export WHATSAPP_PLATFORM_TO="${WHATSAPP_PLATFORM_TO:-}"

if [ -n "$EVOLUTION_API_URL" ] && [ -n "$EVOLUTION_API_KEY" ]; then
  cyan "   WhatsApp:  Evolution configured (${EVOLUTION_INSTANCE:-?})"
else
  red "   WhatsApp:  NOT SET — only email notifications"
fi
...
ENV_PAIRS=(
  ...
  "WHATSAPP_PLATFORM_TO=${WHATSAPP_PLATFORM_TO}"
)
